...
public class ExternalAlbumService {
    ...
    public List<ExternalAlbumDto> searchAlbums(String query) {
        var musicBrainzAlbums = musicBrainzService.searchAlbums(query);

        return musicBrainzAlbums.stream().map(album -> {
                    String coverUrl = coverArtArchiveService.getCoverUrl(album.musicBrainzId());
